...
function SideBar({setShowSidebar , showSidebar}) {
  ...
  return (
    <div id="sidebar" className="md:w-[250px] w-full bg-[#F9F9F9] md:h-[210vh] fixed md:relative z-[2000] h-full hidden lg:block">
      <div className="flex flex-col md:justify-center pt-4">
        {/* logo */}
        <div className="px-6 flex justify-between border-b-[1px] pb-2">
          <div className="flex md:justify-center">
            <img src={logo} className="w-[80px] h-[30px] md:w-[110px]" alt="Logo" />
            <i className="bx bxs-color text-[#F5CAAB] "></i>
          </div>
          <div>
            <i class="bx bx-x text-[30px] bg-[#F5CAAB] rounded-full md:hidden" onClick={() => setShowSidebar(false)}></i>
          </div>
        </div>
        {/* pages */}
        <div className="pt-2">
